...
migrate((app) => {
  ...
  // This snapshot was generated by PocketBase <= 0.22, where collection
  // fields lived under `schema` and field settings under `options`.
  // PocketBase >= 0.23 expects `fields` with flattened settings instead.
  const idField = {
  ...
  const collections = snapshot.map((item) => {
    const { schema = [], options = {}, ...collection } = item;

    const fields = schema.map((field) => {
      const { options: legacyFieldOptions = {}, ...fieldData } = field;
      const fieldOptions = { ...legacyFieldOptions };

      delete fieldData.unique;

      if (field.type === "number") {
        fieldOptions.onlyInt = Boolean(fieldOptions.noDecimal);
        delete fieldOptions.noDecimal;
      }

      return { ...fieldData, ...fieldOptions };
    });

    if (collection.type === "view") {
      const deletedFieldIndex = fields.findIndex((field) => field.name === "deleted");
      const deletedField = deletedFieldIndex >= 0
        ? fields.splice(deletedFieldIndex, 1)[0]
        : null;
    ...
    if (collection.type === "view") {
      collection.viewQuery = options.query || "";
    }
    ...
    if (collection.type === "auth") {
      ...
      if (options.allowUsernameAuth) {
        collection.fields.splice(1, 0, {
      ...
      collection.manageRule = options.manageRule;
      collection.authRule = options.onlyVerified ? "verified = true" : "";
      collection.passwordAuth = {
        enabled: identityFields.length > 0,
        identityFields,
      };
      collection.oauth2 = {
        enabled: Boolean(options.allowOAuth2Auth),
      };
    }
